<x-app-layout :title="'Edit Barang'">
                        ...
                        <form method="POST" action="{{ route('barang.update', $barang->id) }}" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <div id="vendor-container">
                                    @foreach($barang->barangVendors as $index => $barangVendor)
                                        <div class="vendor-item border p-3 mb-3 rounded">
                                            <div class="row">
                                                <div class="col-md-3">
                                                    <label class="form-label">Vendor</label>
                                                    <select class="form-select vendor-select" name="vendors[{{ $index }}][id]" required>
                                                        <option value="">Pilih Vendor</option>
                                                        @foreach ($vendor as $v)
                                                            <option value="{{ $v->id }}" 
                                                                {{ $v->id == $barangVendor->vendor_id ? 'selected' : '' }}>
                                                                {{ $v->nama_vendor }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                ...
                                                <div class="col-md-3">
                                                    ...
                                                    <div class="input-group">
                                                        ...
                                                        <input type="text" class="form-control harga-satuan" 
                                                               name="vendors[{{ $index }}][harga_satuan]" 
                                                               value="{{ number_format($barangVendor->harga_satuan, 0, ',', '.') }}" 
                                                               required>
                                                    </div>
                                                </div>
                                                <div class="col-md-2">
                                                    ...
                                                    <div class="input-group">
                                                        <input type="number" class="form-control persentase-keuntungan" 
                                                               name="vendors[{{ $index }}][persentase_keuntungan]" 
                                                               value="{{ $barangVendor->persentase_keuntungan }}" 
                                                ...
                                                <div class="col-md-3">
                                                    ...
                                                    <div class="input-group">
                                                        ...
                                                        <input type="text" class="form-control harga-jual" 
                                                               name="vendors[{{ $index }}][harga_jual]" 
                                                               value="{{ number_format($barangVendor->harga_jual, 0, ',', '.') }}" 
                                                               readonly>
                                                    </div>
                                                </div>
                                                <div class="col-md-1">
                                                    ...
                                                    <button type="button" class="btn btn-danger d-block remove-vendor" 
                                                            style="{{ count($barang->barangVendors) > 1 ? 'display: block;' : 'display: none;' }}">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            let vendorIndex = {{ count($barang->barangVendors) }};
            ...
            // Initialize existing vendor items
            document.querySelectorAll('.vendor-item').forEach(vendorItem => {
                const hargaSatuanInput = vendorItem.querySelector('.harga-satuan');
                const persentaseInput = vendorItem.querySelector('.persentase-keuntungan');
                const removeBtn = vendorItem.querySelector('.remove-vendor');
                const vendorSelect = vendorItem.querySelector('.vendor-select');
                ...
                // Add change event listener to existing vendor selects
                vendorSelect.addEventListener('change', function() {
                    updateVendorOptions();
                });
            ...
            // Initial update of vendor options
            updateVendorOptions();
